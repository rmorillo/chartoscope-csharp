...
{
    public class FixedProfitSrategy: ExtensibleStrategyBase
    {
        public const string IDENTITY_CODE = "basic_strategy";
        private ISignal signal = null;

        public FixedProfitSrategy(ISignal signal)
        {
            this.signal = signal;
        ...
        protected override void ReceiveQuote(double bidPrice, double askPrice)
        {
            if (this.IsInPosition && (ProfitOrLossInPips <= -50))
            {
                Exit();
            }
        }
